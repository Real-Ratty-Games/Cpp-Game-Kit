...
void Gamepad::LoadConfig()
{
	if (SDL_AddGamepadMappingsFromFile("Data/Gamepads.ini") == -1)
	{
		...
		throw new GamepadException(errmsg);
	}
}
...
GamepadException::GamepadException(strgv msg)
{
	mMsg = msg;
}

void GamepadException::Throw(strgv msg)
{
	throw GamepadException(msg);
}

cstrg GamepadException::what() const noexcept
{
	return mMsg.c_str();
}
