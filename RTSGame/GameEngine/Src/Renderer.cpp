...
void Renderer::LoadTextureFromFile(Texture& texture, strgv filename, uint64 flags, strgv texturename, bool flipUV)
{
	...
	if (!data)
	{
		...
		throw new DrawException(errmsg);
	}
...
void Renderer::LoadTextureFromMemory(Texture& texture, std::vector<uint8>& data, uint64 flags, strgv texturename)
{
	if (!data.empty())
	{
		...
		if (bgfx::isValid(texture.Handle))
			bgfx::setName(texture.Handle, texturename.data());
		else throw new DrawException("Failed loading texture from memory!");
	...
	else throw new DrawException("Failed loading texture from memory; Data was empty!");
}
...
void Renderer::PrepareSpriteInstancing(Sprite* sprite, InstanceData& idata, std::vector<Transform2D>& tdata)
{
	...
	for (uint i = 0; i < drawnIns; i++)
	{
		...
		mdl = Transformation::Translate(mdl, vec3(transf.Location.X, transf.Location.Y, 0.0f));													// move to world pos
		mdl = Transformation::Translate(mdl, rotPiv);																	// mov pivot to origin
		mdl = Transformation::Rotate(mdl, vec3(0.0f, 0.0f, 1.0f), Transformation::ToRadians(transf.Rotation));			// rotate
		mdl = Transformation::Translate(mdl, -rotPiv);																	// move pivot back
		mdl = Transformation::Scale(mdl, vec3(rscale.X, rscale.Y, 1.0f));												// scale
...
void Renderer_DrawTexture(Texture* texture, vec2& rotpiv, vec2& size, Transform2D& transformation)
{
	...
	mat4 mdl = mat4::Identity();
	mdl = Transformation::Translate(mdl, vec3(transformation.Location.X, transformation.Location.Y, 0.0f));													// move to world pos
	mdl = Transformation::Translate(mdl, rotPiv);																	// mov pivot to origin
	mdl = Transformation::Rotate(mdl, vec3(0.0f, 0.0f, 1.0f), Transformation::ToRadians(transformation.Rotation));	// rotate
	mdl = Transformation::Translate(mdl, -rotPiv);																	// move pivot back
	mdl = Transformation::Scale(mdl, vec3(rscale.X, rscale.Y, 1.0f));												// scale
